					...
					<div class="card" style="margin-bottom: 0px">
						<div class="card-body" style="padding: 1.25rem">
							<div class="form-group row">
								...
								<label for="" class="col-sm-2 col-form-label">Gram
									Panchayat<span class="text-danger"> *</span></label>
								<div class="col-sm-2">
							...
							<div class="form-group row">
								...
								<div class="col-sm-2">
									<select class="form-control custom-select" id="ddlvillage"
										onchange="bindplotno(this.value)">
										<option>-- Select --</option>
									</select>
								</div>
								<label id="lbllndtype" for="" class="col-sm-2 col-form-label">Plot
									No</label>
								<div class="col-sm-2" id="divlndtype">
									<select class="form-control custom-select" id="txtplotno">
										<option>-- Select --</option>
									</select>
								</div>
							...
							<div class="form-group row" id="serchdiv" style="display: none">
								<label for="" class="col-sm-1 col-form-label font-11">Village</label>
								<div class="col-sm-2">
									<input class="form-control text-box single-line" type="text"
										id="txtvilname" />

								</div>
								<label for="" class="col-sm-1 col-form-label font-11">Name/Regd
									No./Plot No.</label>
								<div class="col-sm-2">
									<input class="form-control text-box single-line"
										id="searchtext" placeholder="Name/Regd No./Plot No."
	                                        ...
	                                        <thead>
	                                            <tr>
	                                                <th>Sl.No.</th>
	                                                    ...
	                                                    <th>CRZ Status</th>
	                                                    <th>CAA Status</th>
	                                                    <th class="custom-hide">Action</th>
	                                            </tr>
	                                        </thead>
                               ...
                               $('#myData tbody').empty();
                                ...
                                $.each(data, function (i, item1) {
                                    ...
                                    var rows = "<tr>" +
                                        ...
                                        "<td>" + item1[5] + "</td>" +
                                        "<td>" + item1[6] + "</td>" +
                                        "<td class='hiddata'><div class='dropdown'><a class='dropdown-toggle' id='dropdownMenuButton' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'><i class='fa fa-cog'></i></a> <div class='dropdown-menu' aria-labelledby='dropdownMenuButton'><a data-toggle='tab' href='#tab1' role='tab' value=" + item1[4] + " onclick='return MapPlot(" + item1[4] + ")' class='dropdown-item nav-link'>View in Map</a></div></div></td>" +
                                        "</tr>";
                                    $('.tbody').append(rows);
                                });
                                var a=data.length;
                                $("#result").text(a + " Records Found");    
                                if ($('#txtplotno').val() != "ALL"){
                                	MapPlot($("#txtplotno").val());
                                	bindecoinfowindow(plot_current_layer,'',''); 
                                }else{
                                   zoomallplotlayer();                                 
                                   bindecoinfowindow(plot_current_layer,'','');                                  
...
function gpzoom() {
    ...
    var gjFormat =  new ol.format.GeoJSON({
 	...
 	fetch('/geojson/getGpJson?'+param, {
  	...
  	// gplayer.getSource().updateParams(
  	// 		{
  	// 		 'CQL_FILTER' : "tehname='" + $('#ddltehesil').val() + "' && gpname='" + $('#ddlgp').val() + "'"
  	// 		});
  	// gplayer.setVisible(true);
  }
function villzoom() {
      //alert($('#ddlvillage').val());
    if(vill_current_layer!=null){
        map.removeLayer(vill_current_layer);
    }
  	villstdylayer.setVisible(false);
    ...
    var styles =new ol.style.Style({
  	...
  	// gplayer.setVisible(true);
  	// villstdylayer.getSource().updateParams(
  	// 		{'CQL_FILTER' : "tehname='" + $('#ddltehesil').val()
  	// 					+ "' && gpname='" + $('#ddlgp').val()
  	// 					+ "' && villname='" + $('#ddlvillage').val() + "'"
  	// 		});
  	// villstdylayer.setVisible(true);
  }
function zoomallplotlayer(){
	if(plot_current_layer!=null){
		//alert("hello")
		map.removeLayer(plot_current_layer);
	} 
	  var vectorSource = new ol.source.Vector();
	    var styles =new ol.style.Style({
	         stroke : new ol.style.Stroke({
	    ...
	    var param='dist='+$('#ddldist').val()+'&tehsil='+$('#ddltehesil').val()+'&gp='+$('#ddlgp').val()+'&vill='+$('#ddlvillage').val();
	  	fetch('/geojson/getAllPlotGeoJson?'+param, {
	  		method : 'GET',
	  	}).then(function(response) {
	  		return response.json();
	  	}).then(function(json) {
...
function tablesum()
{
    ...
    $("#myData tr").not(':first').not(':last').each(function () {
        sum1 += parseFloat($(this).find("td:eq(5)").text());
        sum2 += parseFloat($(this).find("td:eq(7)").text());
        sum3 += parseFloat($(this).find("td:eq(8)").text());
    });
