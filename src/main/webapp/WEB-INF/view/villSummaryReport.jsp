...
<div class="page-loader" id="loaderdiv" style="display: none">
	<img src="<spring:url value="/images/loader.gif"/>" alt="loader" />
</div>
	...
	<div class="container-fluid">
		<div class="page-title-box">
			<div class="row align-items-center">
				<div class="col-sm-6">
					<h4 class="page-title">Village Wise Statics Report</h4>
				...
				<div class="col-sm-6">
					<ol class="breadcrumb">
					    ...
					    <li class="breadcrumb-item active"> Village Report</li>
					</ol>
				</div>
			</div>
		</div>
...
$(document).ready(function() {
	...
	$.ajax({
		url : "/report/getAllVillageSummaryReport",
		data :{'district' : 'ALL'},
		type : "GET",
		success:function(data) {
			...
			rows = '<tr><td></td><td></td><td>Total</td><td id="sum1"></td><td id="sum2"></td><td id="sum3"></td><td id="sum4"></td><td id="sum5"></td><td="sum6"></td><td id="sum7"></td><td id="sum8"></td><td id="sum9"></td></tr>';
	...
	function tablesum() {
        ...
        $("#myData tr").not(':first').not(':last').each(function () {
            sum1 += parseFloat($(this).find("td:eq(3)").text());
            sum2 += parseFloat($(this).find("td:eq(4)").text());
            sum3 += parseFloat($(this).find("td:eq(5)").text());
            sum4 += parseFloat($(this).find("td:eq(6)").text());
            sum5 += parseFloat($(this).find("td:eq(7)").text());
            sum6 += parseFloat($(this).find("td:eq(8)").text());
            sum7 += parseFloat($(this).find("td:eq(8)").text());                
            sum8 += parseFloat($(this).find("td:eq(9)").text());
            sum9 += parseFloat($(this).find("td:eq(10)").text());
...
function bindvillage(tehsil) {
	
	$.ajax({
		url : "/getVillagebyDistTehsil",
		data : {
...
$("#sub").click(function(){	
		...
		$.ajax({
			url : "/report/getVillageSummaryReport",
			data : {					
