																	...
																	<div id="accordion3">
																	 <a id="layerDivprivate4kclip" href="#collapse12" class="text-dark collapsed" data-toggle="collapse" aria-expanded="false" aria-controls="collapse12">
																		...
																		<div id="collapse9" class="collapse"
																			aria-labelledby="heading9" data-parent="#accordion3">
																			<form class="form-horizontal" action="">
																				<div class="form-group-sm row">
																					<div class="custom-control custom-checkbox">
																	...
																	<div class="maplayer-div">
																		<div id="legendDiv" class="widget-main">
																			 <h6 class="mt-2 header-title mb-2" id="icon_admin_heading">Administrative Boundaries</h6>
	                                                                            ...
	                                                                            <span id="icon_flc" style="" class="legend-text-style"><img src="<spring:url value="/images/fishing_LandCentre.jpg"/>" /></span> 
	                                                                            <span id="icon_cyclone_shelters" style="" class="legend-text-style"><img src="<spring:url value="/images/icon-cyclone-shelters.jpg"/>" /></span> 
	                                                                            ...
	                                                                            <span id="icon_port" style="" class="legend-text-style"><img src="<spring:url value="/images/port.jpg"/>" /></span>
	                                                                            <span id="icon_fishing_harbour" style="" class="legend-text-style"><img src="<spring:url value="/images/fishing_harbour.jpg"/>" /></span>
	                                                                            <span id="icon_light_house" style="" class="legend-text-style"><img src="<spring:url value="/images/light_house.jpg"/>" /></span> 
                                                                                <br/>
...
var waterbody_crz_ivb = new ol.layer.Image({
  source: new ol.source.ImageWMS({
	...
	var  khurdha_feasible = new ol.layer.Image({
        source: new ol.source.ImageWMS({
            url: baseURLWMS,
            params: {
                'LAYERS': 'COAST:Khurdha_Feasible_Land',
	...
	function zoomlyr(layername) {
		var urlname = baseURLWMS+'?service=WMS&version=1.1.0&request=GetCapabilities';
			...
			success : function(data) {
				...
				for (var i = 0, len = Layers.length; i < len; i++) {
					...
					if (layerobj.Name == layername) {
						...
						var ext = new ol.proj.transformExtent(extent, ol.proj
								.get('EPSG:32645'), ol.proj.get('EPSG:4326'));
						var center = new ol.extent.getCenter(ext);
						map.setView(new ol.View({
							center : ol.proj
									.fromLonLat([ center[0], center[1] ]),
							zoom : 10
						}));
		...
		measuringTool
				.on(
						...
						function(event) {
							...
							listener = event.feature
									.on(
											'change',
											function(event) {
												var geom = event.target;
												var measurement = geometryType === 'Polygon' ? event.target
														.getGeometry()
														.getArea()
														: event.target
																.getGeometry()
												...
												//console.clear();
												var measurementFormatted = measurement > 100 ? (measurement / 1000)
														.toFixed(2)
														+ 'km'
														: measurement
																.toFixed(2)
																+ 'm';
												console
														.log(measurementFormatted);
												resultElement
														.html(measurementFormatted
																+ html);
		...
		if(distname!=""){
			$.ajax({
				...
				success : function(data) 
				{
					...
					var options = '';
					$('#ddltehesil').html('');
					$('#ddltehesil').append($("<option></option>").attr("value", "ALL").text("All Tehsil"));
					$('#ddlgp').html('');
		...
		if(tehsil!="ALL"){
			...
			$.ajax({
				...
				success : function(data) {
					...
					var options = '';
					$('#ddlgp').html('');
					$('#ddlgp').append($("<option></option>").attr("value", "ALL").text("All GP"));
					$('#ddlvillage').html('');
					$('#ddlvillage').append($("<option></option>").attr("value", "").text("--Select--"));					
					for (var i = 0; i < result.length; i++) 
					{
						options += '<option value="' +result[i] + '">' + result[i]+ '</option>';
						$('#ddlgp').append($("<option></option>").attr("value", result[i]).text(result[i]));
					}
				},
				error : function(errormessage) 
				{
		...
		if(gp!="ALL"){
			...
			$.ajax({
				...
				success : function(data) {
					...
					var result = JSON.parse(data);
					var options = '';
					$('#ddlvillage').append($("<option></option>").attr("value", "ALL").text("All Village"));
					for (var i = 0; i < result.length; i++) 
					{
						options += '<option value="' +result[i] + '">' + result[i]+ '</option>';
						$('#ddlvillage').append($("<option></option>").attr("value", result[i]).text(result[i]));
					}
				},
				error : function(errormessage) 
				{
